...
const Board = ({ gameId }) => {
  ...
  const [colorToPlayerIdMap, setColorToPlayerIdMap] = useState({});
  const [playerIdToColorMap, setPlayerIdToColorMap] = useState({});
  const [playerNameMap, setPlayerNameMap] = useState({});

  const [userPlayerId, setUserPlayerId] = useState(null);
  ...
  const getBoardState = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setPlayerIdToColorMap(playerIdToColorMapping);
  ...
  const handlePlayerMapping = (coinStateData) => {
    ...
    coinStateData.forEach((element) => {
      colorToPlayerIdMapping[element.color] = element.player_id;
      playerIdToColorMapping[element.player_id] = element.color;
    });
    return [colorToPlayerIdMapping, playerIdToColorMapping];
  };

  const handlePlayerNameMapping = (playerNames, playerIdToColorMapping) => {
    const mapping = {};
